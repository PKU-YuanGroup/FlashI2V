...
from mindspeed_mm.utils.utils import build_iterations
...
def model_provider(pre_process=True, post_process=True):
    """Builds the model."""
    args = get_args()
    print_rank_0("building FlashI2V model ...")
    model = FlashI2VModel(args.mm.model)
...
def forward_step(data_iterator, model):
    """Forward step."""
    batch, video, start_frame, prompt_ids, video_mask, prompt_mask = {}, None, None, None, None, None
    skip_encode = False
    if mpu.is_pipeline_first_stage():
        ...
        if get_batch_interval > 1 and len(batches) >= 1:
            video, start_frame, prompt_ids, video_mask, prompt_mask = [], [], [], [], []
            for single_batch in batches:
                ...
                start_frame.append(single_batch.pop(START_FRAME, None))
                prompt_ids.append(_prompt_ids)
                prompt_mask.append(_prompt_mask)
        elif len(batches) == 1:
            batch = batches[0]
            video = batch.pop(VIDEO, None)
            start_frame = batch.pop(START_FRAME, None)
            prompt_ids = batch.pop(PROMPT_IDS, None)
            video_mask = batch.pop(VIDEO_MASK, None)
            prompt_mask = batch.pop(PROMPT_MASK, None)

    output_tensor_list = model(video, prompt_ids, start_frame=start_frame, video_mask=video_mask, prompt_mask=prompt_mask, skip_encode=skip_encode, **batch)
