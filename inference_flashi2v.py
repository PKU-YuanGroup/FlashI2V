...
from mindspeed_mm.tasks.inference.pipeline import FlashI2VPipeline
...
def prepare_pipeline(args, device):
    ...
    predictor = PredictModel(args.predictor).get_model()
    if ori_args.load is not None:
        load_checkpoint([predictor], None, None, strict=False)
    predictor = predictor.to(device, args.predictor.dtype).eval()
...
def main():
    ...
    images = load_images(args.image, False) if hasattr(args, "image") else None
