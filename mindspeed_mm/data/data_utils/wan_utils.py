...
from mindspeed_mm.data.data_utils.data_transform import TemporalRandomCrop, filter_resolution
from mindspeed_mm.data.data_utils.utils import AbstractDataProcessor, AbstractDataFilter, TextProcessor, read_ann_txt
from mindspeed_mm.data.data_utils.video_reader import VideoReader, VideoReader_contextmanager
from mindspeed_mm.data.data_utils.image_reader import ImageReader
...
class WanVideoProcessor(AbstractDataProcessor):
    def __init__(
        ...
        self.video_reader_class = VideoReader(video_reader_type)
        ...
        if self.transform_pipeline is not None:        
            self.video_transforms = get_transforms(is_video=True, train_pipeline=self.transform_pipeline)
    ...
    def read_one_sample(self, path, meta_info=None):
        with VideoReader_contextmanager(self.video_reader_class, path, self.video_layout_type, num_threads=1) as video_reader:
...
class WanImageProcessor(AbstractDataProcessor):
    def __init__(
        ...
        self.transform_pipeline = transform_pipeline
        if self.transform_pipeline is not None:        
            self.image_transforms = get_transforms(is_video=False, train_pipeline=self.transform_pipeline)
