...
from mindspeed_mm.models.common.normalize import FP32LayerNorm, RMSNorm
...
@torch.autocast("cuda", enabled=False)
def gate(x: torch.Tensor, gate: torch.Tensor, residual: torch.Tensor):
...
class WanDiT(MultiModalModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print('------------------------------in wandit, recompute config------------------------------')
        print(f"recompute_granularity: {self.recompute_granularity}")
        print(f"distribute_saved_activations: {self.distribute_saved_activations}")
        print(f"recompute_method: {self.recompute_method}, recompute_layers: {self.recompute_layers}")
        print(f"recompute_skip_core_attention: {self.recompute_skip_core_attention}, recompute_num_layers_skip_core_attention: {self.recompute_num_layers_skip_core_attention}")
        print(f"attention_async_offload: {self.attention_async_offload}")
        print(f"use {self.fp32_dtype} as high precision to forward some layers")
        print('---------------------------------------------------------------------------------------')
        ...
        if self.pre_process:
            ...
            self.time_embedding = nn.Sequential(
                FP32Linear(self.freq_dim, self.hidden_size),
                nn.SiLU(),
                FP32Linear(self.hidden_size, self.hidden_size),
            )
            # time emb projection
            self.time_projection = nn.Sequential(
                nn.SiLU(), FP32Linear(self.hidden_size, self.hidden_size * 6)
            )
        ...
        if self.pre_process and model_type in ["i2v", "flf2v"]:
            self.img_emb = MLPProj(self.img_dim, self.hidden_size, model_type == 'flf2v', clip_token_len)
        ...
        **kwargs,
        ...
        if self.pre_process:
            ...
            if self.model_type in ["i2v", "flf2v"]:
                ...
                clip_embedding = self.img_emb(i2v_clip_feature.float())
    ...
    def pipeline_set_prev_stage_tensor(self, input_tensor_list, extra_kwargs):
        ...
        (prev_output, prompt, prompt_emb, time_emb, times, prompt_mask,
         latents, prior_dist, video_mask) = input_tensor_list
        predictor_input_list = [prev_output, timesteps, prompt, None, prompt_mask]
        training_loss_input_list = [latents, prior_dist, None]
    ...
    def pipeline_set_next_stage_tensor(self, input_list, output_list, extra_kwargs=None):
        ...
        """
        latents, prior_dist, video_mask = input_list

        return list(output_list) + [latents, prior_dist, video_mask]
    ...
    def initialize_pipeline_tensor_shapes():
        ...
        data_cfg = args.mm.data.dataset_param
        hidden_size = model_cfg.predictor.hidden_size
        height = getattr(data_cfg, "sample_height", 480)
        width = getattr(data_cfg, "sample_width", 832)
        num_frames = getattr(data_cfg, "sample_num_frames", 49)
        vae_scale_factor = getattr(model_cfg.predictor, "vae_scale_factor", [4, 8, 8])
        latent_size = ((num_frames + 3) // vae_scale_factor[0], height // vae_scale_factor[1], width // vae_scale_factor[2])
...
class WanDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.norm1 = FP32LayerNorm(self.hidden_size, eps=eps, elementwise_affine=False)
        self.norm2 = FP32LayerNorm(self.hidden_size, eps=eps, elementwise_affine=False)
        self.norm3 = FP32LayerNorm(self.hidden_size, eps=eps, elementwise_affine=True)
    ...
    def _before_self_attention(
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            self.modulation.to(dtype=self.fp32_dtype, device=latents.device) + time_emb
        ).chunk(6, dim=1)

        self_attn_input = modulate(
            self.norm1(latents).to(self.fp32_dtype), shift_msa, scale_msa
        ).to(latents.dtype)
    ...
    def _after_self_attention(
        ...
        latents = gate(self_attn_out.to(self.fp32_dtype), gate_msa, latents.to(self.fp32_dtype)).to(latents.dtype)
        # cross attention
        ...
        modu_out = modulate(self.norm2(latents).to(self.fp32_dtype), shift_mlp, scale_mlp).to(latents.dtype)
        # ffn
        latents = gate(self.ffn(modu_out).to(self.fp32_dtype), gate_mlp, latents.to(self.fp32_dtype)).to(latents.dtype)
...
class Head(nn.Module):
    ...
    def __init__(
        ...
        self.head = FP32Linear(dim, out_dim * math.prod(patch_size))
        self.modulation = nn.Parameter(torch.randn(1, 2, dim) / dim**0.5)

    @torch.autocast("cuda", enabled=False)
    def forward(self, latents, times):
        shift, scale = (
            self.modulation.to(dtype=self.fp32_dtype, device=latents.device) + times
        ).chunk(2, dim=1)
        out = self.head(modulate(self.norm(latents).float(), shift, scale))
        return out


class MLPProj(nn.Module):

    def __init__(self, in_dim: int, out_dim: int, flf_pos_emb=False, clip_token_len=257):
        super().__init__()
