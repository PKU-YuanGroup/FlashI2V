...
class SoRAModel(nn.Module):
    ...
    def forward(self, video, prompt_ids, video_mask=None, prompt_mask=None, skip_encode=False, **kwargs):
        ...
        if self.pre_process:
            ...
            q_sample_output = self.diffusion.q_sample(latents, model_kwargs=kwargs, mask=video_mask)
            x_t = q_sample_output.pop("x_t", None)
            prior_dist = q_sample_output.pop("prior_dist", None)
            sigmas = q_sample_output.pop("sigmas", None)
            timesteps = q_sample_output.pop("timesteps", None)
            predictor_input_latent, predictor_timesteps, predictor_prompt = x_t, timesteps, prompt
    ...
    def compute_loss(
        self, model_output, latents, prior_dist, video_mask, **kwargs
    ):
