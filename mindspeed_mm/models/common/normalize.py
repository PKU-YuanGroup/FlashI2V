...
class FP32LayerNorm(nn.LayerNorm):
    def __init__(
        ...
        **kwargs
        ...
        self.dim = (dim,)
        super().__init__(self.dim, eps=eps, elementwise_affine=elementwise_affine, **kwargs)
...
class AELayerNorm(FP32LayerNorm):
    def __init__(self, num_channels, eps=1e-6, elementwise_affine=True, *args, **kwargs) -> None:
        super().__init__(dim=(num_channels,), eps=eps, elementwise_affine=elementwise_affine, **kwargs)
...
def normalize(in_channels, num_groups=32, eps=1e-6, affine=True, norm_type="groupnorm", gather=False, *args, **kwargs):
    if not gather:
        if norm_type == "groupnorm":
            ...
            return AELayerNorm(num_channels=in_channels, eps=eps, *args, **kwargs)
        elif norm_type == "layernorm":
            return FP32LayerNorm(in_channels, eps=eps, elementwise_affine=affine, *args, **kwargs)
        elif norm_type == "rmsnorm":
            return RMSNorm(dim=in_channels, eps=eps, *args, **kwargs)
        else:
